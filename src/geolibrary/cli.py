...
@click.group()
@click.version_option(version=version("geolibrary"))
def main() -> None:
...
@click.option("--latitude", required=True, type=float, help="Latitude coordinate")
@click.option("--longitude", required=True, type=float, help="Longitude coordinate")
def add(name: str, description: str | None, latitude: float, longitude: float) -> None:
    """Add a new location."""
    try:
...
def get(location_id: int | None, name: str | None) -> None:
    ...
    try:
        with LocationRepository() as repo:
            location = (
                repo.get_location_by_id(location_id) if location_id
                else repo.get_location_by_name(name)
            )
...
def search_area(points: str) -> None:
    ...
    try:
        ...
        if len(point_strings) < 3:
            click.echo("Error: Polygon must have at least 3 points", err=True)
            sys.exit(1)
        ...
        for point_str in point_strings:
            try:
                ...
                click.echo(
                    f"Error: Invalid point format '{point_str}'. Expected 'lat,lon'",
                    err=True,
                )
                sys.exit(1)
        ...
        with LocationRepository() as repo:
            ...
            if not locations:
                click.echo("No locations found within the specified polygon area.")
                return

            click.echo(f"Found {len(locations)} location(s) within polygon:\n")
            for loc in locations:
                click.echo(
                    f"ID: {loc.id} | Name: {loc.name} | "
                    f"Lat: {loc.latitude}, Lon: {loc.longitude}"
